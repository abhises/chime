import ScyllaDb from '../src/ScyllaDb.js';
import fs from 'fs';
...
async function testSimpleTransactionAlternative() {
  ...
  try {
    ...
    // Verify both items
    const retrieved1 = await ScyllaDb.getItem('products', { id: 'simple001', category: 'electronics' });
    const retrieved2 = await ScyllaDb.getItem('products', { id: 'simple002', category: 'books' });
    ...
    const updated1 = await ScyllaDb.updateItem('products', { id: 'simple001', category: 'electronics' }, {
      price: 179.99,
      updatedAt: Date.now()
    });
    ...
    const updated2 = await ScyllaDb.updateItem('products', { id: 'simple002', category: 'books' }, {
      stock: 30,
      updatedAt: Date.now()
    });
...
async function testBatchWriteItem() {
  ...
  try {
    ...
    for (const item of items) {
      const retrieved = await ScyllaDb.getItem('products', { id: item.id, category: item.category });
...
async function testBatchGetItem() {
  ...
  try {
    ...
    console.log(`✅ Retrieved ${result.filter(item => item !== null).length} items`);
...
async function testMultipleOperationsSimulation() {
  ...
  try {
    ...
    // Verify both items
    const retrieved1 = await ScyllaDb.getItem('products', { id: 'multi001', category: 'electronics' });
    const retrieved2 = await ScyllaDb.getItem('products', { id: 'multi002', category: 'books' });
    ...
    console.log('Updating item 1...');
    const updated1 = await ScyllaDb.updateItem('products', { id: 'multi001', category: 'electronics' }, {
      price: 179.99,
      updatedAt: Date.now()
    });
    console.log('✅ Item 1 updated:', updated1?.price);
    ...
    console.log('Updating item 2...');
    const updated2 = await ScyllaDb.updateItem('products', { id: 'multi002', category: 'books' }, {
      stock: 30,
      updatedAt: Date.now()
    });
    console.log('✅ Item 2 updated:', updated2?.stock);
    ...
    await ScyllaDb.deleteItem('products', { id: 'multi001', category: 'electronics' });
    console.log('✅ Item 1 deleted');
    
    console.log('Deleting item 2...');
    await ScyllaDb.deleteItem('products', { id: 'multi002', category: 'books' });
    console.log('✅ Item 2 deleted');
    
    // Verify deletion
    const deleted1 = await ScyllaDb.getItem('products', { id: 'multi001', category: 'electronics' });
    const deleted2 = await ScyllaDb.getItem('products', { id: 'multi002', category: 'books' });
...
async function testDescribeTable() {
  ...
  try {
    ...
    if (tableDescription.Table) {
      ...
      console.log('   Full Table object:', JSON.stringify(tableDescription.Table, null, 2));
    } else {
      console.log('   No Table object found in response');
      console.log('   Full response:', JSON.stringify(tableDescription, null, 2));
    }
    ...
    console.log('   Item Count:', table.ItemCount || table.NumberOfItems || 'Not available');
    console.log('   Table Size:', table.TableSizeBytes || table.SizeBytes || 'Not available');
    console.log('   Table Status:', table.TableStatus || 'Not available');
    console.log('   Creation Date:', table.CreationDateTime || table.CreationDate || 'Not available');
    ...
    const knownProps = ['TableName', 'ItemCount', 'TableSizeBytes', 'TableStatus', 'CreationDateTime', 'KeySchema', 'AttributeDefinitions'];
    const otherProps = Object.keys(table).filter(key => !knownProps.includes(key));
...
async function testGSI() {
  ...
  try {
    ...
    try {
      ...
      const brandQuery = await ScyllaDb.query('products_with_gsi', 
        'brand = :brand',
        { ':brand': 'Apple' },
        { IndexName: 'BrandIndex' }
      );
...
async function testTransactWrite() {
  ...
  try {
    ...
    console.log(`   Operations completed: ${result.results.filter(r => r.success).length}`);
    
    // Verify items were created/updated
    const item1 = await ScyllaDb.getItem('products', { id: 'transact001', category: 'electronics' });
    const item2 = await ScyllaDb.getItem('products', { id: 'transact002', category: 'books' });
...
async function testTransactGet() {
  ...
  try {
    ...
    console.log(`   Items retrieved: ${result.results.filter(r => r.item !== null).length}`);
    
    result.results.forEach((itemResult, index) => {
      if (itemResult.item) {
        console.log(`   ${index + 1}. ${itemResult.item.name}: $${itemResult.item.price}`);
      } else {
...
async function testTransactWriteWithRollback() {
  ...
  try {
    ...
    try {
      ...
      const originalItem = await ScyllaDb.getItem('products', { id: 'rollback001', category: 'electronics' });
      const newItem = await ScyllaDb.getItem('products', { id: 'rollback002', category: 'books' });
...
async function testTransactWriteMixedOperations() {
  ...
  try {
    ...
    // Verify results
    const newItem = await ScyllaDb.getItem('products', { id: 'mixed003', category: 'clothing' });
    const updatedItem = await ScyllaDb.getItem('products', { id: 'mixed001', category: 'electronics' });
    const deletedItem = await ScyllaDb.getItem('products', { id: 'mixed002', category: 'books' });
...
async function main() {
  try {
    ...
    const tableConfigs = JSON.parse(fs.readFileSync('./config/tables.json', 'utf8'));
